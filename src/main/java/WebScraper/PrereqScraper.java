...
public class PrereqScraper {
    ...
    public static void main(String args[]) {
        PrereqScraper prereqScraper = new PrereqScraper();
        prereqScraper.getPrereq();
    }

    private PrereqScraper() {
    ...
    private void getPrereq() {
        try {
            for (Course course : courses.values()) {
                JSONObject jsonObject = buildPostObject(course);
                HttpURLConnection connection = buildConnection(jsonObject);
                JSONArray coursesJSON = getResponse(connection);
                JSONObject courseJSON = coursesJSON.getJSONObject(0);
                String prereqsString = courseJSON.getString("classPreReqs");

                if (prereqsString.equals("")) {
                    continue;
                }

                ArrayList<Course> courses = new ArrayList<>();
                String prereqsCopy = prereqsString;
                for (Department department : departments.values()) {
                    String abbreviation = department.getAbbreviation() + " ";
                    while (prereqsCopy.contains(abbreviation)) {
                        int index = prereqsCopy.indexOf(abbreviation);
                        if (index != 0 && !prereqsCopy.contains(" " + abbreviation)) {
                            prereqsCopy = prereqsCopy.substring(index + abbreviation.length());
                            continue;
                        }
                        prereqsCopy = prereqsCopy.substring(index + abbreviation.length());
                        String code = "";
                        for (int i = 0; i < prereqsCopy.length(); i++) {
                            char c = prereqsCopy.charAt(i);
                            if (c == ',' || c == ';' || c == ' ' || c == '.' || c == '<' || c == '/' || c == ')') {
                                break;
                            }
                            code += c;
                        ...
                        if (code.length() != 0 && Character.isDigit(code.charAt(0))) {
                            Course prereq = databaseConnection.getCourse(department.getId(), code);
                            if (prereq != null && !course.equals(prereq)) {
                                databaseConnection.exportPrerequisites(course, prereq);

                            }
                        }
                        prereqsCopy = prereqsCopy.replaceFirst(code, "");
                    }
                }
            }
        } catch (Exception e) {
    ...
    private JSONObject buildPostObject(Course course) {
        ...
        jsonObject.put("selectedDepartment", course.getDepartment().getAbbreviation());
        jsonObject.put("courseNumber", course.getCode());
        return jsonObject;
    }

    private HttpURLConnection buildConnection(JSONObject jsonObject) {
        try {
    ...
    private JSONArray getResponse(HttpURLConnection connection) {
        try {
            ...
            return new JSONObject(response.toString()).getJSONArray("response");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
